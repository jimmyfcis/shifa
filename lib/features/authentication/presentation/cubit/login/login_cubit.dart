...
class AuthCubit extends Cubit<AuthState> {
  ...
  AuthCubit({required this.updateProfileUseCase, required this.loginUseCase,required this.registerUseCase}) : super(AuthInitial());
  ...
  Future<void> updateProfile({required User user}) async {
    try {
      emit(AuthLoading());
      final loginResponse = await updateProfileUseCase(
          user: user);
      emit(AuthSuccess(loginResponse));
    } catch (error) {
      final failure = ErrorHandler.handle(error);
